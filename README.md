...
| 📊 整體損益 | 自訂帳號分組：可新增/重命名/刪除/鎖定分組、拖曳排序、帳號勾選（同帳號不可跨組重複）；分組各有今日損益+%、未實現損益、總成本、總市值小計（teal），末尾美股/台股整體合計以金色加大顯示；變更需明確按「儲存」才生效 |
...
| 📉 K 線頁面 | 點擊任一代號（Cards、盤前卡片、P&L 表格代號欄）進入 K 線；開盤中預設「盤中」tab（X 軸固定 US 9:30–16:00 ET / TW 9:00–13:30），盤外（盤前/後/休市）預設「24H」tab（X 軸為此時此刻往前 24 小時滾動視窗）；24H~5日 tab 皆標示盤中時間段；30 秒自動重整 + 手動重整按鈕 |
| 🌐 市場總覽 | 📈 市場 tab：US 市場即時漲幅/跌幅/成交量前 25 名（Yahoo Finance 全市場 Screener）；TW 熱門股票排序 |
| ₿ 加密貨幣 | 自訂觀察清單（新增/移除幣種）、卡片 + 表格、排序切換，清單持久化儲存 |
